...
def load_data(subfolder,opt,generator=False):
...
#                 image = image / 127.5 - 1
        ...
        return image_array
    if subfolder=='T1-T2':
        file_path=opt['dataset_path']
        trainA_path = os.path.join(file_path, subfolder, 'TrainT1')
        trainB_path = os.path.join(file_path, subfolder, 'TrainT2')
        testA_path = os.path.join(file_path, subfolder, 'TestT1')
        testB_path = os.path.join(file_path, subfolder, 'TestT2')
    else:
        file_path=opt['dataset_path']

        trainA_path = os.path.join(file_path, subfolder, 'trainCT')
        trainB_path = os.path.join(file_path, subfolder, 'trainMRI')
        testA_path = os.path.join(file_path, subfolder, 'testCT')
        testB_path = os.path.join(file_path, subfolder, 'testMRI')
...
def load_negative_data(image_folder,opt):

    if image_folder == 'T1-T2':
        MRI_path = os.path.join(opt['dataset_path'],image_folder,'TrainT2')
        CT_path = os.path.join(opt['dataset_path'],image_folder,'TrainT1')
        ...
        for count in range(opt['batch_size']):
            trip_image_MRI = mpimg.imread(os.path.join(opt['dataset_path'],image_folder,'TrainT2', file_MRI[count]))
            ...
            trip_image_CT = mpimg.imread(os.path.join(opt['dataset_path'],image_folder,'TrainT1', file_CT[count]))
        ...
        MRI_path = os.path.join(opt['dataset_path'],image_folder,'trainMRI')
        CT_path = os.path.join(opt['dataset_path'],image_folder,'trainCT')
        file_MRI = os.listdir(MRI_path)
        file_CT = os.listdir(CT_path)
        shuffle(file_MRI)
        shuffle(file_CT)
        ...
        image_array_mri = np.empty((opt['batch_size'],) + ((256,256)) + (1,))
        for count in range(opt['batch_size']):
            trip_image_MRI = mpimg.imread(os.path.join(opt['dataset_path'],image_folder,'trainMRI', file_MRI[count]))
            trip_image_MRI = resize(trip_image_MRI,(256,256))
            trip_image_MRI = trip_image_MRI[np.newaxis,:, :, np.newaxis]
            trip_image_MRI = trip_image_MRI * 2 - 1
            trip_image_CT = mpimg.imread(os.path.join(opt['dataset_path'],image_folder,'trainCT', file_CT[count]))
            trip_image_CT = resize(trip_image_CT,(256,256))
            trip_image_CT = trip_image_CT[np.newaxis,:, :, np.newaxis]
            trip_image_CT = trip_image_CT * 2 - 1
            image_array_ct[count, :, :, :] = trip_image_CT
            image_array_mri[count, :, :, :] = trip_image_MRI
    return image_array_ct,image_array_mri
...
def save_model(opt, model, epoch):
    ...
    directory = os.path.join('/data/jiayuan', 'saved_models', opt['date_time'])
